...
class JumpIfEq implements InstructionInterface
{
    ...
    {
        //get the label, symbols and types
        ...
        if($symb1type == DataTypeEnum::nil || $symb2type == DataTypeEnum::nil) //comparable to the EQ Instruction
        {
            ...
            {
                $executionManager->setInstructionPointer($label); //jump to the label if the types are the same
            }
        }
        else if($symb1type == DataTypeEnum::bool || $symb1type == DataTypeEnum::int || $symb1type == DataTypeEnum::string)
        {
            if($symb1type == $symb2type)
            {
                ...
                if($jump)
                {
                    $executionManager->setInstructionPointer($label); //jump to the label if the values are the same
                }
            }
            else
            {
